...
- 同步快照：客户端通过 `GET /v1/workspaces/{workspace_id}/snapshot` 获取工作区、自选股、分析、AI 分析师问答、风控、纸面订单、纸面组合和同步事件。
...
- 本地持久化：Core 使用 SQLite 保存账号、设备、工作区、自选股、同步事件、AI 分析、AI 分析师问答、风控判定、纸面订单、纸面组合和审计日志。
