...
- 角色门禁：审批列表、审批/拒绝和 kill switch 只允许 `admin` 或 `risk_manager` 访问；普通用户仍可做研究、回测和纸面交易。
...
- 当前同步链路不授予实盘交易权限。
- 账号密码登录、PBKDF2 密码哈希、开发期 MFA 占位码和角色门禁已经打通最小链路，但仍不是生产身份系统。
- 设备会话返回的 `dubhe_dev_` 令牌已用于本地设备认证和撤销，但仍属于开发级令牌；生产版必须补齐正式 OIDC/企业身份、刷新令牌、真实 MFA、密码策略、角色分配 UI 和审计策略。
- 工作区快照、自选股写入、增量事件、WebSocket 实时同步、纸面订单、审批请求和 kill switch 端点已经要求设备 token；跨工作区 token 会被拒绝。
...
- 增加正式 OIDC/企业身份、刷新令牌、真实 MFA、管理员角色分配 UI 和审计日志。
