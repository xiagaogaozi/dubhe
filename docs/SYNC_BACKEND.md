...
- 本地持久化：Core 使用 SQLite 保存账号、设备、工作区、自选股、同步事件、AI 分析、风控判定、纸面订单和审计日志。
...
- 工作区快照、自选股写入、增量事件、WebSocket 实时同步、风控评估、风险决策列表、纸面订单、审批请求、kill switch、角色分配和审计日志端点已经要求设备 token；跨工作区 token 会被拒绝。
