...
if (-not $macosDmg -and -not $macosZip) {
    Write-MissingArtifactNote `
        ...
        -Lines @(
            ...
            "请先运行 Authorize-Dubhe-GitHub-Actions.cmd 和 Activate-Dubhe-GitHub-Actions.cmd，再手动运行 Theia Desktop Packages，或在真实 Mac 上执行对应构建流程。",
...
if (-not (Test-Path $iosApp) -and -not $iosIpa) {
    Write-MissingArtifactNote `
        ...
        -Lines @(
            ...
            "请先运行 Authorize-Dubhe-GitHub-Actions.cmd 和 Activate-Dubhe-GitHub-Actions.cmd，再手动运行 Mobile Companion Packages，或在 macOS + Xcode 环境执行对应构建流程。",
...
Write-Launcher -Path (Join-Path $kitRoot "15-Authorize-Dubhe-GitHub-Actions-This-PC.cmd") -Title "Authorize Dubhe GitHub Actions on this PC" -TargetScript (Join-Path $repoRoot "Authorize-Dubhe-GitHub-Actions.cmd")
Write-Launcher -Path (Join-Path $kitRoot "16-Activate-Dubhe-GitHub-Actions-This-PC.cmd") -Title "Activate Dubhe GitHub Actions on this PC" -TargetScript (Join-Path $repoRoot "Activate-Dubhe-GitHub-Actions.cmd")
