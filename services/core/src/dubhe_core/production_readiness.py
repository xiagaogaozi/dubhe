    ...
    items = [
        ...
        *news_items,
        _item(
            ...
            status="pass" if status.auth.production_ready else "fail",
            blocking=not status.auth.production_ready,
            evidence_zh=status.auth.message_zh,
            next_step_zh=status.auth.next_step_zh,
        ),
        _item(
            item_id="production_storage",
            category_zh="云同步与存储",
            requirement_zh="生产环境必须使用 PostgreSQL/TimescaleDB、Redis、对象存储、备份和迁移策略。",
            status="pass" if status.storage.production_ready else "fail",
            blocking=not status.storage.production_ready,
            evidence_zh=status.storage.message_zh,
            next_step_zh=status.storage.next_step_zh,
        ),
        _item(
