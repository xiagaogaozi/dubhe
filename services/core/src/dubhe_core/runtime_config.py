...
LOCAL_CONFIG_DEFINITIONS = [
    ...
    LocalConfigDefinition(
        key="DUBHE_AUTH_MODE",
        label_zh="生产目标认证模式",
        description_zh="生产目标认证模式；当前代码仍运行 local_dev，后续接入 OIDC 后才可通过门禁。",
        group_zh="生产身份",
        placeholder="oidc",
        setup_hint_zh="生产准备时填写 oidc；仅填写该项不会切换当前本地登录链路。",
        restart_required=True,
    ),
    LocalConfigDefinition(
        key="DUBHE_OIDC_ISSUER_URL",
        label_zh="OIDC Issuer URL",
        description_zh="企业身份或云身份供应商的 OIDC issuer 地址。",
        group_zh="生产身份",
        placeholder="https://login.example.com/realms/dubhe",
        setup_hint_zh="从 Auth0、Keycloak、Azure AD、Okta 等 OIDC 租户复制。",
        restart_required=True,
    ),
    LocalConfigDefinition(
        key="DUBHE_OIDC_CLIENT_ID",
        label_zh="OIDC Client ID",
        description_zh="Dubhe 客户端在 OIDC 租户里的 Client ID。",
        group_zh="生产身份",
        placeholder="dubhe-client",
        setup_hint_zh="需要配套 redirect URI、MFA 策略和允许的回调域名。",
        restart_required=True,
    ),
    LocalConfigDefinition(
        key="DUBHE_OIDC_CLIENT_SECRET",
        label_zh="OIDC Client Secret",
        description_zh="OIDC 客户端密钥；不会在接口响应中回显。",
        group_zh="生产身份",
    ...
    LocalConfigDefinition(
        key="DUBHE_OIDC_REDIRECT_URI",
        label_zh="OIDC Redirect URI",
        description_zh="OIDC 登录回调地址。",
        group_zh="生产身份",
        placeholder="dubhe://auth/callback 或 https://api.example.com/auth/callback",
        setup_hint_zh="需要同时覆盖桌面端、移动端和后端回调策略。",
        restart_required=True,
    ),
    LocalConfigDefinition(
        key="DUBHE_SESSION_SIGNING_KEY",
        label_zh="会话签名密钥",
        description_zh="生产会话/JWT 签名密钥；不会在接口响应中回显。",
        group_zh="生产身份",
        placeholder="至少 32 字节随机密钥",
        setup_hint_zh="必须使用密钥管理系统生成和轮换；不要复用演示密钥。",
        secret=True,
        restart_required=True,
    ),
    LocalConfigDefinition(
        key="DUBHE_REFRESH_TOKEN_TTL_DAYS",
        label_zh="刷新令牌有效期",
        description_zh="生产刷新令牌有效天数。",
        group_zh="生产身份",
        placeholder="30",
        setup_hint_zh="需要和设备撤销、MFA 重验证和异常登录策略一起制定。",
        restart_required=True,
    ),
    LocalConfigDefinition(
        key="DUBHE_OIDC_MFA_POLICY_URL",
        label_zh="OIDC MFA 策略文档",
        description_zh="生产 MFA、条件访问和管理员强验证策略文档地址。",
        group_zh="生产身份",
        placeholder="https://docs.example.com/dubhe/identity-mfa-policy",
        setup_hint_zh="生产门禁需要能追溯真实 MFA 和管理员访问策略。",
        restart_required=True,
    ),
    LocalConfigDefinition(
        key="DUBHE_IDENTITY_RUNBOOK_URL",
        label_zh="身份系统 Runbook",
        description_zh="账号开通、角色变更、会话撤销和应急处理 runbook 地址。",
        group_zh="生产身份",
        placeholder="https://docs.example.com/dubhe/identity-runbook",
        setup_hint_zh="生产门禁需要覆盖管理员、风控管理员、普通用户的生命周期。",
        restart_required=True,
    ),
    LocalConfigDefinition(
...
def write_local_config_values(path: Path, values: dict[str, str]) -> None:
    ...
    lines.extend(["", "# Production identity readiness probe."])
    _append_named_lines(
        lines,
        known_values,
        [
            ...
            "DUBHE_REFRESH_TOKEN_TTL_DAYS",
            "DUBHE_OIDC_MFA_POLICY_URL",
            "DUBHE_IDENTITY_RUNBOOK_URL",
        ],
    )
