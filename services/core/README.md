...
- 策略模板目录：`GET /v1/strategy/templates` 返回新闻情绪、公告确认、自选股再平衡等小白模板，标注适用市场、数据依赖、风险护栏和参考开源范式。
- 从模板生成策略草案：`POST /v1/strategy/drafts/from-template` 会把模板、标的和市场转换成仅允许纸面验证的 `StrategyDraft`，并写入工作区同步事件。
