...
安装后会出现 `Start Dubhe`、`Start Dubhe LAN`、`Connect Dubhe Mobile`、`Build Dubhe User Kit`、`Prepare Dubhe Delivery`、`Dubhe Mobile Guide`、`Dubhe Install Guide`、`Configure Dubhe`、`Setup Dubhe MFA`、`Accept Dubhe`、`Verify Dubhe Audit`、`Check Dubhe`、`Smoke Dubhe`、`Test Dubhe Services`、`Check Dubhe Production`、`Export Dubhe Production Pack`、`Stop Dubhe Core` 十七个快捷方式；双击入口和快捷方式安装脚本保持纯 ASCII，以避免 Windows PowerShell 5 在中文编码下解析失败。
