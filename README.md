...
安装后会出现 `Start Dubhe`、`Start Dubhe LAN`、`Connect Dubhe Mobile`、`Build Dubhe User Kit`、`Dubhe Mobile Guide`、`Dubhe Install Guide`、`Configure Dubhe`、`Accept Dubhe`、`Check Dubhe`、`Smoke Dubhe`、`Test Dubhe Services`、`Check Dubhe Production`、`Stop Dubhe Core` 十三个快捷方式；双击入口和快捷方式安装脚本保持纯 ASCII，以避免 Windows PowerShell 5 在中文编码下解析失败。
