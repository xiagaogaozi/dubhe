...
- [Core API CI 模板](docs/ci/core.yml)：可由 `Activate-Dubhe-GitHub-Actions.cmd` 激活到 `.github/workflows/core.yml`，用于 Core lint、测试、smoke 和 OpenAPI 合约检查。
- [Theia Desktop Packages CI 模板](docs/ci/theia-desktop.yml)：可由 `Activate-Dubhe-GitHub-Actions.cmd` 激活到 `.github/workflows/theia-desktop.yml`，用于构建 Windows setup/portable 以及 macOS arm64/x64 dmg/zip 未签名产物。
- [Mobile Companion Packages CI 模板](docs/ci/mobile.yml)：可由 `Activate-Dubhe-GitHub-Actions.cmd` 激活到 `.github/workflows/mobile.yml`，用于生成 Android debug APK、可选签名 AAB 与 iOS no-codesign app bundle。
...
安装后会出现 `Start Dubhe`、`Start Dubhe LAN`、`Connect Dubhe Mobile`、`Build Dubhe User Kit`、`Prepare Dubhe Delivery`、`Verify Dubhe Delivery`、`Import Dubhe CI Artifacts`、`Export Dubhe Release Evidence`、`Activate Dubhe GitHub Actions`、`Dubhe Mobile Guide`、`Dubhe Install Guide`、`Configure Dubhe`、`Setup Dubhe MFA`、`Accept Dubhe`、`Verify Dubhe Audit`、`Check Dubhe`、`Smoke Dubhe`、`Test Dubhe Services`、`Check Dubhe Production`、`Export Dubhe Production Pack`、`Stop Dubhe Core` 二十一个快捷方式；双击入口和快捷方式安装脚本保持纯 ASCII，以避免 Windows PowerShell 5 在中文编码下解析失败。
...
如果 GitHub Actions 尚未启用，先双击 `Activate-Dubhe-GitHub-Actions.cmd`。GitHub CLI token 需要 `workflow` scope；如果脚本提示权限不足，先运行 `gh auth refresh -h github.com -s workflow`，授权完成后再双击。启用后从 GitHub Actions 下载到 macOS/iOS/Android/Windows artifact ZIP，把它们放进 `.dubhe-run\ci-artifacts`，再双击 `Import-Dubhe-CI-Artifacts.cmd`。它会自动解压可识别的 artifact、复制 `.dmg/.zip/Runner.app/.ipa/APK/AAB/EXE` 到对应构建目录，并重新生成最新交付 ZIP。
