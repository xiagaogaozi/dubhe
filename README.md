...
安装后会出现 `Start Dubhe`、`Start Dubhe LAN`、`Dubhe Mobile Guide`、`Configure Dubhe`、`Check Dubhe`、`Smoke Dubhe`、`Stop Dubhe Core` 七个快捷方式；双击入口和快捷方式安装脚本保持纯 ASCII，以避免 Windows PowerShell 5 在中文编码下解析失败。
...
手机真机可以先双击 `Open-Dubhe-Mobile-Guide.cmd` 查看安装与连接向导；实际连接前再双击 `Start-Dubhe-LAN.cmd`，让 Core 监听局域网地址。启动后体检会显示类似 `http://192.168.x.x:8000` 的地址，把它填到移动端登录页的“Core 地址”即可；如果 Windows 防火墙弹窗出现，请允许专用网络访问。Android 模拟器仍可使用 `http://10.0.2.2:8000`。
