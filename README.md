...
安装后会出现 `Start Dubhe`、`Start Dubhe LAN`、`Build Dubhe User Kit`、`Dubhe Mobile Guide`、`Dubhe Install Guide`、`Configure Dubhe`、`Accept Dubhe`、`Check Dubhe`、`Smoke Dubhe`、`Test Dubhe Services`、`Check Dubhe Production`、`Stop Dubhe Core` 十二个快捷方式；双击入口和快捷方式安装脚本保持纯 ASCII，以避免 Windows PowerShell 5 在中文编码下解析失败。
...
把这台电脑交给非技术用户试用前，可以双击 `Accept-Dubhe.cmd`。它会自动确保 Core 运行，依次跑本机体检、主链路 smoke 和外部 AI/新闻源状态检查，并把结果写入 `.dubhe-run\local-acceptance.txt` 和 `.dubhe-run\local-acceptance.json`。没有填写授权 key 时，验收会显示“需配置”，但不会把本地演示链路判为失败；需要强制外部服务全部通过时，可用命令行加 `-RequireExternalServices`。
...
给这台电脑上的非技术用户试用时，可以双击 `Build-Dubhe-User-Kit.cmd`。它会生成 `.dubhe-run\user-kits\Dubhe-User-Kit-...`，复制当前 Windows setup/portable、Android APK/AAB、中文向导、体检/验收报告，并生成指向当前仓库的“本机启动/配置/验收/体检”双击入口；这仍是内测交付包，不等同于签名生产安装器。
...
`run-local-acceptance.ps1` 会自动启动 Core，然后串起本机体检、主链路 smoke 和外部服务状态检查，适合交给不会命令行的用户前做一次“这台电脑能不能用”的兜底验收；报告会写入 `.dubhe-run\local-acceptance.txt` 和 `.dubhe-run\local-acceptance.json`。
