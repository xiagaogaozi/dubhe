...
- [Dubhe Theia Desktop](apps/theia-desktop/README.md)：Eclipse Theia Desktop 正式壳骨架，包含 Electron target application package 和 Dubhe Theia extension；当前已通过本地 Theia build，会承载并逐步替换 React/Vite 原型。
- [Dubhe Companion](apps/mobile/README.md)：Flutter iOS/Android 移动端源码骨架，包含本地账号登录、新闻雷达、中文 AI 影响分析、纸面组合和审批操作入口；当前仍需安装 Flutter 后生成平台工程与安装包。
