...
安装后会出现 `Start Dubhe`、`Start Dubhe LAN`、`Connect Dubhe Mobile`、`Build Dubhe User Kit`、`Prepare Dubhe Delivery`、`Verify Dubhe Delivery`、`Import Dubhe CI Artifacts`、`Export Dubhe Release Evidence`、`Dubhe Mobile Guide`、`Dubhe Install Guide`、`Configure Dubhe`、`Setup Dubhe MFA`、`Accept Dubhe`、`Verify Dubhe Audit`、`Check Dubhe`、`Smoke Dubhe`、`Test Dubhe Services`、`Check Dubhe Production`、`Export Dubhe Production Pack`、`Stop Dubhe Core` 二十个快捷方式；双击入口和快捷方式安装脚本保持纯 ASCII，以避免 Windows PowerShell 5 在中文编码下解析失败。
...
从 GitHub Actions 下载到 macOS/iOS/Android/Windows artifact ZIP 后，把它们放进 `.dubhe-run\ci-artifacts`，再双击 `Import-Dubhe-CI-Artifacts.cmd`。它会自动解压可识别的 artifact、复制 `.dmg/.zip/Runner.app/.ipa/APK/AAB/EXE` 到对应构建目录，并重新生成最新交付 ZIP。
