...
- [Core API CI 模板](docs/ci/core.yml)：先用 `Authorize-Dubhe-GitHub-Actions.cmd` 给 GitHub CLI 补 `workflow` scope，再用 `Activate-Dubhe-GitHub-Actions.cmd` 激活到 `.github/workflows/core.yml`，用于 Core lint、测试、smoke 和 OpenAPI 合约检查。
- [Theia Desktop Packages CI 模板](docs/ci/theia-desktop.yml)：授权并激活后用于构建 Windows setup/portable 以及 macOS arm64/x64 dmg/zip 未签名产物。
- [Mobile Companion Packages CI 模板](docs/ci/mobile.yml)：授权并激活后用于生成 Android debug APK、可选签名 AAB 与 iOS no-codesign app bundle。
...
- `Import-Dubhe-CI-Artifacts.cmd`：导入 GitHub Actions 下载的四端构建产物，放入正确目录后重新生成交付 ZIP。
- `Export-Dubhe-Release-Evidence.cmd`：导出中文发行证据包，汇总最新交付 ZIP、默认/严格四端校验、本机体检和生产门禁状态。
- `Authorize-Dubhe-GitHub-Actions.cmd`：给 GitHub CLI 补 `workflow` scope，用于允许仓库创建/更新 `.github/workflows`。
- `Activate-Dubhe-GitHub-Actions.cmd`：复制 CI 模板到 `.github/workflows`，并在 GitHub CLI 具备 `workflow` scope 时提交推送，触发四端构建。
...
安装后会出现 `Start Dubhe`、`Start Dubhe LAN`、`Connect Dubhe Mobile`、`Build Dubhe User Kit`、`Prepare Dubhe Delivery`、`Verify Dubhe Delivery`、`Import Dubhe CI Artifacts`、`Export Dubhe Release Evidence`、`Authorize Dubhe GitHub Actions`、`Activate Dubhe GitHub Actions`、`Dubhe Mobile Guide`、`Dubhe Install Guide`、`Configure Dubhe`、`Setup Dubhe MFA`、`Accept Dubhe`、`Verify Dubhe Audit`、`Check Dubhe`、`Smoke Dubhe`、`Test Dubhe Services`、`Check Dubhe Production`、`Export Dubhe Production Pack`、`Stop Dubhe Core` 二十二个快捷方式；双击入口和快捷方式安装脚本保持纯 ASCII，以避免 Windows PowerShell 5 在中文编码下解析失败。
...
如果 GitHub Actions 尚未启用，先双击 `Authorize-Dubhe-GitHub-Actions.cmd` 给 GitHub CLI 补 `workflow` scope，再双击 `Activate-Dubhe-GitHub-Actions.cmd`。启用后从 GitHub Actions 下载到 macOS/iOS/Android/Windows artifact ZIP，把它们放进 `.dubhe-run\ci-artifacts`，再双击 `Import-Dubhe-CI-Artifacts.cmd`。它会自动解压可识别的 artifact、复制 `.dmg/.zip/Runner.app/.ipa/APK/AAB/EXE` 到对应构建目录，并重新生成最新交付 ZIP。

需要给项目负责人、测试人员或商务/合规看“当前到底能不能交付”时，双击 `Export-Dubhe-Release-Evidence.cmd`。它会生成 `.dubhe-run\release-evidence`，包含可打开的 `RELEASE-EVIDENCE.html`、最新 ZIP 摘要、默认验证、严格四端验证、本机体检和生产门禁证据。
...
.\scripts\authorize-github-workflow-scope.ps1
.\scripts\activate-github-actions.ps1 -CommitAndPush
.\scripts\import-ci-artifacts.ps1 -PrepareDelivery -VerifyDelivery
.\scripts\export-release-evidence.ps1
