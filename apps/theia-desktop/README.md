...
- 默认打开 `Dubhe` 主视图，当前已经是中文 IDE 式工作台骨架，包含活动栏、自选列表、新闻雷达、AI 分析师、策略草案、回测报告、纸面组合、审批中心和任务日志区域。
- 当前主视图保留现有 React/Vite 原型和 Dubhe Core API 文档入口，后续会把按钮和面板逐步接到 Core API。
